...
<div class="col col-sm col-md{{$inputSize}} col-lg{{$inputSize}}">
    ...
    <input type="number" class="form-control{{ $errors->has($field) ? ' is-invalid' : '' }} {{$css}}" name="{{$name}}" id="{{$id}}" min="{{$numMin}}" max="{{$numMax}}" step="{{$numStep}}" value="{{ isset($inputValue) ? $inputValue : old($field) }}" {{ $required ? 'required' : '' }}  {{ $autofocus ? 'autofocus' : '' }} {{ $disabled ? 'disabled="disabled"' : '' }}>
